...
# --- Step 9: Personalize Git Configuration ---
echo ""
echo "[9/9] Personalizing Git Configuration..."
read -p "Enter your First Name: " first_name
read -p "Enter your Last Name: " last_name
read -p "Enter your Personal Email: " personal_email
read -p "Enter your Work Email: " work_email

git_user_name="${first_name} ${last_name}"

echo "  Updating Git configuration..."
sed -e "s/GIT_USER_NAME/$git_user_name/g" \
    -e "s/GIT_PERSONAL_EMAIL/$personal_email/g" \
    -e "s/# \[includeIf/\[includeIf/g" \
    -e "s/#     path = ~\/\.gitconfig-work/    path = ~\/\.gitconfig-work/g" \
    "$HOME/.gitconfig" > "$HOME/.gitconfig.tmp"

cat "$HOME/.gitconfig.tmp" > "$HOME/.gitconfig"
rm "$HOME/.gitconfig.tmp"

echo "  Generating work Git configuration (~/.gitconfig-work)..."
