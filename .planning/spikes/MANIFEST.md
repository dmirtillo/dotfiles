...
| 028 | ponytail-audit-validation | standard | Given the dotfiles repo, when running `ponytail-audit`, then it successfully identifies bloat or over-engineering that manual cleanup missed | PENDING | ponytail, audit, cleanup |
| 029 | officecli-skill-hybrid-test | standard | Given the existing `officecli` GSD skill, when tasked to extract and edit a document, then it successfully uses the `markitdown` hybrid approach without error | PENDING | officecli, markitdown, integration |
| 030 | mise-global-tools | standard | Given `mise`, when configuring global tools (`mise use -g`), then they are correctly shimmed and available to chezmoi scripts without complex path management | PENDING | mise, tools, chezmoi |
| 031 | zsh-startup-profiling | standard | Given the current `.zshrc`, when profiled via `zprof` or `hyperfine`, then startup time is < 50ms and the lazy-loading strategy is confirmed effective | PENDING | zsh, performance, profiling |
