...
TASKS_DIR="/Users/dmirtillo/projects/software/dotfiles/.planning/spikes/032-gsd-core-skill-collision/test-config/tasks"
if [ -d "$TASKS_DIR" ]; then
  STALE_COUNT=$( (find "$TASKS_DIR" -maxdepth 1 -type d -mtime +1 2>/dev/null || true) | wc -l )
  if [ "$STALE_COUNT" -gt 0 ]; then
    echo "⚠️  Found $STALE_COUNT stale task directories in /Users/dmirtillo/projects/software/dotfiles/.planning/spikes/032-gsd-core-skill-collision/test-config/tasks/"
    echo "   These are leftover from crashed subagent sessions."
    echo "   Run: rm -rf /Users/dmirtillo/projects/software/dotfiles/.planning/spikes/032-gsd-core-skill-collision/test-config/tasks/*  (safe — only affects dead sessions)"
  fi
fi
```
